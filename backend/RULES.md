...
| # | Quy tắc |
|---|---|
| 3.1 | **[BẮT BUỘC]** Handler/service inject `IUnitOfWork<HelpdeskDbContext>`. **Không** inject DbContext trực tiếp, **không** `new SqlConnection`, **không** `new DbContext`. |
...
| 4.5 | **[BẮT BUỘC]** Đổi schema phải đi kèm migration: `dotnet ef migrations add <Tên> -p src/Helpdesk.Persistence -s src/Helpdesk.Persistence`. Không sửa migration đã merge; muốn sửa thì tạo migration mới. |
