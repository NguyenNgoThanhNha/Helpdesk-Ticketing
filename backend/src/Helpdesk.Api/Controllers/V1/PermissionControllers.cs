...
using Helpdesk.Application.Features.V1.Roles.Commands;
using Helpdesk.Application.Features.V1.Roles.DTOs;
using Helpdesk.Application.Features.V1.Roles.Queries;
using Helpdesk.Application.Features.V1.Users.Commands;
using Helpdesk.Application.Features.V1.Users.DTOs;
using Helpdesk.Application.Features.V1.Users.Queries;
...
{
    [HttpGet("assignees")]
    [HasPermission("TICKET_ASSIGN:U|TICKET:R")]
    public async Task<ActionResult<IReadOnlyList<UserSummaryDto>>> Assignees(CancellationToken ct) =>
        Ok(await Mediator.Send(new GetAssigneesQuery(), ct));
