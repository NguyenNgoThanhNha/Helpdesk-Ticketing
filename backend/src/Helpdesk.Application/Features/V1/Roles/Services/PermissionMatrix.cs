...
namespace Helpdesk.Application.Features.V1.Roles.Services;
...
public static class PermissionMatrix
{
    ...
    public static async Task<IReadOnlyList<ActivityPermissionDto>> ToDtosAsync(
        IUnitOfWork<HelpdeskDbContext> unitOfWork, IEnumerable<CrudPermission> rows, CancellationToken ct)
    {
...
{
    public ActivityPermissionInputsValidator(IUnitOfWork<HelpdeskDbContext> unitOfWork)
    {
        RuleFor(x => x)
