namespace Helpdesk.Application.Features.V1.Users.DTOs
{
...
namespace Helpdesk.Application.Features.V1.Users.Services
{
    using Helpdesk.Application.Features.V1.Roles.DTOs;
    using Helpdesk.Application.Features.V1.Roles.Services;
    using Helpdesk.Application.Features.V1.Users.DTOs;
    ...
    public static class UserReader
    {
        ...
        public static async Task<UserListItemDto> GetAsync(IUnitOfWork<HelpdeskDbContext> unitOfWork, Guid userId, CancellationToken ct) =>
        ...
        public static async Task<UserPermissionDetailDto> GetPermissionsAsync(
            IUnitOfWork<HelpdeskDbContext> unitOfWork, IPermissionService permissionService, Guid userId, CancellationToken ct)
        {
        ...
        public static async Task EnsureNotLastAdminAsync(IUnitOfWork<HelpdeskDbContext> unitOfWork, Guid userId, CancellationToken ct)
        {
        ...
        public static Task<bool> IsAdminAsync(IUnitOfWork<HelpdeskDbContext> unitOfWork, Guid userId, CancellationToken ct) =>
            unitOfWork.Repository<SysUserRole>().AnyAsync(ur => ur.UserId == userId && ur.Role.RoleType == ConstRole.AdminRoleType, ct);
    }
}

namespace Helpdesk.Application.Features.V1.Users.Queries
{
    using Helpdesk.Application.Features.V1.Tickets.Services;
    using Helpdesk.Application.Features.V1.Users.DTOs;
    using Helpdesk.Application.Features.V1.Users.Services;
    using Helpdesk.Domain.Entities.Sys;

    /// <summary>User nhận gán được ticket (TICKET_ASSIGN:R) — cho dropdown Assign / filter Assignee.</summary>
    public sealed record GetAssigneesQuery : IRequest<IReadOnlyList<UserSummaryDto>>;

    public sealed class GetAssigneesQueryHandler(IUnitOfWork<HelpdeskDbContext> unitOfWork)
        : IRequestHandler<GetAssigneesQuery, IReadOnlyList<UserSummaryDto>>
    {
        public async Task<IReadOnlyList<UserSummaryDto>> Handle(GetAssigneesQuery request, CancellationToken ct) =>
            await AssignableUsers.Query(unitOfWork, includeAdmins: true).AsNoTracking()
    ...
    public sealed class SearchUsersQueryHandler(IUnitOfWork<HelpdeskDbContext> unitOfWork)
    ...
    public sealed class GetUserPermissionsQueryHandler(IUnitOfWork<HelpdeskDbContext> unitOfWork, IPermissionService permissionService)
...
namespace Helpdesk.Application.Features.V1.Users.Commands
{
    ...
    public sealed class UpdateUserStatusCommandHandler(
        IUnitOfWork<HelpdeskDbContext> unitOfWork, ICurrentUser currentUser, IPermissionService permissionService, TimeProvider clock)
    ...
    {
        public AssignUserRolesCommandValidator(IUnitOfWork<HelpdeskDbContext> unitOfWork)
        {
    ...
    public sealed class AssignUserRolesCommandHandler(
        IUnitOfWork<HelpdeskDbContext> unitOfWork, ICurrentUser currentUser, IPermissionService permissionService)
    ...
    {
        public UpdateUserPermissionsCommandValidator(IUnitOfWork<HelpdeskDbContext> unitOfWork) =>
            RuleFor(x => x.Activities).NotNull().SetValidator(new ActivityPermissionInputsValidator(unitOfWork));
    }

    public sealed class UpdateUserPermissionsCommandHandler(IUnitOfWork<HelpdeskDbContext> unitOfWork, IPermissionService permissionService)
